...
serve(async (req) => {
  try {
    ...
    // Get all subscriptions with evening enabled
    ...
    for (const sub of subscriptions) {
      try {
        ...
        // Only send if it's 8:00 PM (within 1 minute window)
        if (hour === 20 && minute >= 0 && minute < 1) {
          ...
          const pushResponse = await fetch(
            `${Deno.env.get('SUPABASE_URL')}/functions/v1/send_push`,
            {
              ...
              body: JSON.stringify({
                ...
                payload: {
                  title: 'Reflect',
                  body: 'Do your 90-second reflection',
